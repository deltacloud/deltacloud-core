...
module Deltacloud::Drivers::Mock
  ...
  class MockDriver < Deltacloud::BaseDriver
    ...
    def systems(credentials, opts={})
      ...
      if opts[:id].nil?
        systems = @client.load_all_cimi(:system).map{|sys| CIMI::Service::System.new(opts[:env], :model => CIMI::Model::System.from_json(sys))}
      else
        begin
          systems = [CIMI::Service::System.new(opts[:env], :model => CIMI::Model::System.from_json(@client.load_cimi(:system, opts[:id])))]
        rescue Errno::ENOENT
          return []
        end
      end
      systems.map{|sys| convert_urls(sys.model, opts[:env])}.flatten
      systems
    end
    ...
    def create_system(credentials, opts={})
      ...
      sys_hsh = { "id"=> id,
                  ...
                  "operations" => [{"rel"=>"http://schemas.dmtf.org/cimi/1/action/start", "href"=> "#{id}/start"},
                                   {"rel"=>"http://schemas.dmtf.org/cimi/1/action/stop",  "href"=> "#{id}/stop"},
                                   {"rel"=>"edit", "href"=> id},
                                   {"rel"=>"delete", "href"=> id}]    }
      system = CIMI::Model::System.from_json(JSON.generate(sys_hsh))
      @client.store_cimi(:system, system)
      CIMI::Service::System.new(opts[:env], :model=>system)
    end
